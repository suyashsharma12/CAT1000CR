...
(async function initApp(){
  ...
  function renderSets(filter=state.filter){
    ...
    CR_DATA.sets.forEach(s=>{const done=latestAttempt(s.id),source=(s.source||'').toUpperCase();
      ...
      const el=document.createElement('button');el.type='button';el.className='set-card '+(done?'done':'');el.disabled=!!done;
      el.innerHTML=`<div class="set-no">${s.name}</div><div class="set-source">${s.source}</div><div class="set-status">${done?`✓ Completed · ${done.score} marks · ${done.accuracy}% accuracy`:'Start this set →'}</div>`;
      if(!done)el.addEventListener('click',()=>startSet(s));grid.appendChild(el);
    });
  ...
  function updateStats(){const a=attempts();$('completedCount').textContent=completedSetCount();$('totalMarks').textContent=a.length?a.reduce((x,y)=>x+y.score,0):'—';$('strikeRate').textContent=a.length?Math.round(a.reduce((x,y)=>x+y.accuracy,0)/a.length)+'%':'—';const next=CR_DATA.sets.find(s=>!attempts().some(a=>a.setId===s.id));if($('nextSetBtn')){if(next){$('nextSetBtn').disabled=false;$('nextSetBtn').textContent='START NEXT SET →';}else{$('nextSetBtn').disabled=true;$('nextSetBtn').textContent='ALL SETS COMPLETED ✓';}}}
